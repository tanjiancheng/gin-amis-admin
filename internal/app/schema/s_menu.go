...
//FillMenuActionAndResource() 填充菜单动作和资源列表
func (a Menus) FillMenuAndResourceAction(mActions map[string]MenuActions, mResources map[string]MenuActionResources) Menus  {
	for _, item := range a {
		if v, ok := mActions[item.ID]; ok {
			v.FillResources(mResources)
			item.Actions = v
		}
	}
	return a
