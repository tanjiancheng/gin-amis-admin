...
func Init(ctx context.Context, opts ...Option) (func(), error) {
	...
	// 初始化依赖注入器
	injectorObj, injectorCleanFunc, err := injector.BuildInjector()
	if err != nil {
		return nil, err
	}

	//初始化数据
	err = injector.InitGormData(ctx, injectorObj)
	if err != nil {
		return nil, err
	}

	// 初始化HTTP服务
	httpServerCleanFunc := InitHTTPServer(ctx, injectorObj.Engine)
